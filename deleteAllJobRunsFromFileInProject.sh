#/bin/zsh
USAGE="$(basename "$0") project_id job_id filename"
PROJECT_ID=$1
ASSET_ID=$2
...
#    echo "Job_Run_ID:" $RUN_ID
    ...
    then
        ...
        cpdctl job run delete --project-id $PROJECT_ID --job-id $ASSET_ID --run-id $RUN_ID >>/dev/null 2>&1
        #If the job run wasn't already completed, then can force the delete by uncommenting following line
        #cpdctl asset delete --project-id $PROJECT_ID --asset-id $RUN_ID >>/dev/null 2>&1
   else
